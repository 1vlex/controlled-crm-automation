...
## 10. Что должно идти в Git

Оставить в Git:

- исходный код `.py`;
- `rpa_ui/`;
- `odoo_ocr_docker/` без локального `.env`;
...
- `.env` и любые реальные ключи;
- `logs/`, `artifacts/`, `full_eval_runs/`, `ablation_runs/`, `pipeline_runs_*`;
- текст ВКР и LaTeX-сборку;
- презентации и локальные отчеты для руководителя.
